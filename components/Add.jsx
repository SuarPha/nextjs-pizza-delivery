...
const Add = ({ setClose }) => { // we need the state to close/open modal as a prop
    ...
    const handleCreate = async () => {
        ...
        try{
            const uploadRes = await axios.post("http://api.cloudinary.com/v1_1/dg9cf0tux/image/upload", 
            data
            );
  ...
  return (
    <div className={styles.container}>
        <div className={styles.wrapper}>
            ...
            <div className={styles.item}>
                <label className={styles.label}>Desception</label>
                <textarea
                    rows={4}
                    type="text"
            ...
            <div className={styles.item}>
                ...
                type="number"
                placeholder="Small"
                onChange={(e) => changPrice(e, 0)} />
                  <input 
                className={`${styles.input} ${styles.inputSmall}`}
                type="number"
                placeholder="Medium"
                onChange={(e) => changPrice(e, 1)} />
                  <input 
                className={`${styles.input} ${styles.inputSmall}`}
                type="number"
                placeholder="Large"
                onChange={(e) => changPrice(e, 2)} />
                </div>
            </div>
            <div className={styles.item}>
                ...
                <div className={styles.extraItems}>
                    {extraOptions.map(option => (
                        ...
                        className={styles.extraItem}>{option.text}
                        </span>
                    ))}
                </div>
            </div>
