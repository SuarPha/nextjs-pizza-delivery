...
const Index = ({orders, products}) => {
  ...
  const handeStatus = async(id) =>{ //Check order stutus by using async fuction
    ...
    setOrderList([
      res.data,
      ...orderList.filter((order) => order._id !== id), // Delete the prev order, then create the new one with res.data
    ])
    }catch(err){
      console.log(err)
    }
          ...
          <table className={styles.table}>
            ...
            <tbody key={order._id}>
              <tr className={styles.trTtile}>
                ...
                <td>
                  <button onClick={() => handeStatus(order._id)} className={styles.btn}>Next steg</button>
                </td>
              </tr>
            </tbody>
            ))}
