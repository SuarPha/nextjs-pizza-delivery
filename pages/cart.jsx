...
const Cart = () => {
    ...
    const createOrder = async (data) => {
        try {
            // Respone after creating orders
            const res = await axios.post("http://localhost:3000/api/orders", data);
            if (res.status === 201) {
            dispatch(reset());
            router.push(`/orders/${res.data._id}`); // Showing order ids
        }
        }catch(err){
            console.log(err)
        }
    ...
    <div className={styles.container}>
        <div className={styles.left}>
            ...
            <tbody>
                ...
                <tr className={styles.tr} key={product._id}>
                    ...
                    <td>
                        <span className={styles.total}>{product.price * product.quantity} Kr</span>
                    </td>
                </tr>
                ))}
                </tbody>
        ...
        <div className={styles.right}>
            <div className={styles.wrapper}>
                ...
                <div className={styles.totalText}>
                    <b className={styles.totalTextTitle}> Discount: </b> 0 Kr
                </div>
                <div className={styles.totalText}>
